...
if (isset($_GET['download']) && isset($_SESSION['wordsearch_data'])) {
    ...
    $filename = "numbersearch.png";
    $solution_data = [];

    if ($_GET['download'] === 'solution') {
        $solution_data = $data['placed_words'];
        $filename = "numbersearch_solution.png";
    }
...
$errorMessage = '';
$expectedTitle = 'Number Search Maker';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['page_title']) && $_POST['page_title'] === $expectedTitle) {
        ...
        if (empty($raw_words)) {
            $errors[] = "No numbers were entered. Please provide a list of numbers.";
        } else {
            $words = preg_split('/[\s,]+/', $raw_words);
            $words = array_filter(array_map(function($word) {
                $clean_word = preg_replace("/[^0-9]/", "", $word);
                if (strlen($clean_word) > GRID_SIZE) {
                    global $errors;
                    $errors[] = "The number '{$clean_word}' is too long to fit in the " . GRID_SIZE . "x" . GRID_SIZE . " grid.";
                }
                return $clean_word;
            }, $words));

            if (empty($words)) {
                $errors[] = "The input contained no valid numbers. Please use digits only.";
            }
        }
        ...
        $errorMessage = "Please enter the correct page title to generate the Number Search.";
    }
}
...
function fill_empty_cells($grid) {
    for ($r = 0; $r < GRID_SIZE; $r++) {
        for ($c = 0; $c < GRID_SIZE; $c++) {
            if ($grid[$r][$c] === null) {
                // Changed to generate random digits (0-9)
                $grid[$r][$c] = strval(rand(0, 9));
            }
        }
    }
    return $grid;
...
function generate_image($grid, $solution_words = [], $output_mode = 'base64') {
    ...
    if (!FONT_PATH || !file_exists(FONT_PATH)) {
        ...
        for ($r = 0; $r < GRID_SIZE; $r++) {
            for ($c = 0; $c < GRID_SIZE; $c++) {
                $letter = $grid[$r][$c]; // Variable name kept as 'letter' but now represents a digit
...
<html lang="en-US">
    ...
    <head>
        ...
        <!-- Page Details -->
        <title>Number Search Maker | Lunaseeker Press</title>
        <link rel="canonical" href="https://lunaseeker.com/tools/numbersearch">
        <meta name="date" content="2025-07-13">
        <meta name="last-modified" content="2025-07-13">
        <meta name="description" content="Input a list of numbers and click the button to create a number search and its solution!">
        <style>
    ...
    <body>
        ...
        <main>
            ...
            <article id="main">
                ...
                <!-- Page Body -->
                <p id="top" class="p-summary">Input a list of numbers and click the button to create a number search and its solution!</p>
            
                <section>
                    <h2>Create Your Number Search</h2>
                    <form action="/tools/numbersearch" method="post" class="word-search-form">
                        <label for="page_title">Enter Page Title:</label>
                        <br>
                        <input type="text" id="page_title" name="page_title" size="50" required>
                        <br>
                        <label for="words">Enter your numbers (separated by new lines or commas):</label><br>
                ...
                <?php if ($_SERVER['REQUEST_METHOD'] === 'POST'): ?>
                    ...
                    <?php if (!empty($generated_content)): ?>
                        <section class="word-search-results">
                            <h2>Your Number Search</h2>
                            <?php if (!empty($unplaced_words)): ?>
                            <div class="warning-box">
                                <strong>Note:</strong> The following numbers could not be placed (try fewer or shorter numbers):
                                <?php echo htmlspecialchars(implode(', ', $unplaced_words)); ?>.
                            </div>
                            <?php endif; ?>
                            <h3>Numbers To Find</h3>
                            <ul class="word-list">
                            ...
                            <hr>
                            <h3>Puzzle</h3>
                            <img src="<?php echo $generated_content['puzzle_uri']; ?>" alt="Number Search">
                            <hr>
                            <details style="padding-top: 1.2em;">
                                <summary><strong>Click To Reveal The Solution</strong></summary>
                                <h3>Solution</h3>
                                <img src="<?php echo $generated_content['solution_uri']; ?>" alt="Number Search Solution">
                            </details>
